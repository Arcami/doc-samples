...
- Extension Kit:
  - Newly created flows are now internally marked to be subject to new future hard limits. For more information about the current limits, see [Service limits (internal)](#).
  - All services have been migrated to .NET 10.
  - Removed the deprecated **Webhook v1** infrastructure.
  ...
  - Application Insights now logs when the **For each** and **Until** actions reach their total iteration limit.
  - TAF and Tenants API improvements.
  - Various security improvements.

- App Studio:
  - Updated Ripple to version 1.8.1.
...
- App Studio:
  ...
  - A bundler error no longer occurs when a **Data grid** column with the _Datatype_ set to **Value list** contains a plain array in the _Options_ property.
  - A bundler error no longer occurs when a **Stacked list** component contains multiple Liquid template expressions.
